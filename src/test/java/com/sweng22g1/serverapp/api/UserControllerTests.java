...
//TODO: Add test for logging?
	...
	public void GetUserRequestForValidUserReturnsOkStatusCode() throws Exception {
		...
		// Builds the HTTP request.
		RequestBuilder getRequest = MockMvcRequestBuilders.get(url).param("username", username);
	...
	public void CreateUserPostRequestSavesUserAndReturnsOkCode() throws Exception {
		...
		User user = User.builder().username(username).firstname(firstName).lastname(lastName).email(email)
				.password(password).build();

		User userWithGeneratedID = User.builder().username(username).firstname(firstName).lastname(lastName)
				.email(email).password(password).id(1L).build();

		String url = "/api/v1/user";

		// Builds the HTTP request.
		RequestBuilder postRequest = MockMvcRequestBuilders.post(url).param("username", username).param("email", email)
	...
	public void UpdateUserPostRequestAsAdminUpdatesAllFieldsAndReturnsOkCode() throws Exception {
		...
		User user = User.builder().username(username).firstname(firstName).lastname(lastName).email(email)
				.password(password).roles(requestingUserRoles).id(1L).build();

		User requestingUser = User.builder().username(requesterUsername).password(requesterPassword).id(3L)
				.roles(requestingUserRoles).build();
		...
		User updatedUser = User.builder().username(updatedUsername).firstname(updatedFirstName)
				.lastname(updatedLastName).email(updatedEmail).password(updatedPassword).roles(requestingUserRoles)
				.id(1L).build();

		String url = "/api/v1/user/" + username;

		// Builds the HTTP request.
		RequestBuilder postRequest = MockMvcRequestBuilders.post(url).param("newUsername", updatedUsername)
				.param("newEmail", updatedEmail).param("newFirstname", updatedFirstName)
				.param("newLastname", updatedLastName).param("newPassword", updatedPassword)
	...
	public void DeleteUserPostRequestAsAdminDeletesUserAndReturnsOkCode() throws Exception {
		...
		User requestingUser = User.builder().username(requesterUsername).password(requesterPassword).id(2L)
				.roles(requestingUserRoles).build();
		...
		// Builds the HTTP request.
		RequestBuilder postRequest = MockMvcRequestBuilders.delete(url).param("username", username);
	...
	public void DeleteUserPostRequestAsUserForOwnUserDeletesUserAndReturnsOkCode() throws Exception {
		...
		// Builds the HTTP request.
		RequestBuilder postRequest = MockMvcRequestBuilders.delete(url).param("username", requesterUsername);
